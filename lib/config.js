...
const config = {
  ...
  telegram: {
    ...
    botId: (getEnv('BOT_TOKEN', '')).split(':')[0] || '',
    oidcSecret: getEnv('TELEGRAM_OIDC_SECRET', ''),
  },

  s3: {
    endpoint: getEnv('YANDEX_S3_ENDPOINT', 'https://storage.yandexcloud.net'),
    region: getEnv('YANDEX_S3_REGION', 'ru-central1'),
    bucket: getEnv('YANDEX_S3_BUCKET', 'cinefiles-txt-uploads'),
    accessKey: getEnv('YANDEX_S3_ACCESS_KEY', ''),
    secretKey: getEnv('YANDEX_S3_SECRET_KEY', ''),
    cdnUrl: getEnv('MEDIA_CDN_URL', ''),
  },

  tmdb: {
    apiKey: getEnv('TMDB_API_KEY', ''),
    proxyUrl: getEnv('TMDB_PROXY_URL', 'https://cinefiles-txt.vercel.app/api/tmdb'),
...
function validateConfig() {
  if (config.isProd || config.isStaging) {
    if (!config.db.url) throw new Error('DATABASE_URL is required in production/staging');
