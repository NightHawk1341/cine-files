 ...
 * Uses the public /products endpoint which includes primary images.
    ...
    for (const p of catalog) {
      ...
      if (isMatch) {
        ...
        matched.push({
          ...
          imageUrl: p.image || (p.media && p.media.variants && p.media.variants.card) || null,
          url: p.slug ? `${tributeBase}/${p.slug}` : `${tributeBase}/products/${p.id}`,
        });
