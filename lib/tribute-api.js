...
async function fetchTributeProducts(ids) {
  ...
  try {
    ...
    return items.map(p => {
      const hasValidUrl = p.url && !p.url.endsWith('/null') && !p.url.endsWith('/undefined');
      return {
        id: p.id,
        name: p.title || p.name || '',
        price: p.price,
        imageUrl: p.image_url || p.imageUrl || p.image || (p.media && p.media.variants && p.media.variants.card) || null,
        url: hasValidUrl ? p.url : (p.slug ? `${tributeBase}/${p.slug}` : `${tributeBase}/product?id=${p.id}`),
      };
    });
  } catch (err) {
 ...
 * Uses the public /products endpoint which returns JSON with primary images.
