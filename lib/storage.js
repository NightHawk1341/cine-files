...
/**
 * Generate image variants (thumbnail) using sharp.
 * Returns buffer and metadata for the resized image.
 * Skips GIFs (animated) and files smaller than the target width.
 *
 * @param {Buffer} buffer - original image buffer
 * @param {string} mimeType - original MIME type
 * @param {number} [width=400] - target thumbnail width
 * @returns {Promise<{ buffer: Buffer, mimeType: string, suffix: string } | null>}
 */
async function generateThumbnail(buffer, mimeType, width = 400) {
  ...
  try {
    ...
    if (!metadata.width || metadata.width <= width) return null;
    ...
    return { buffer: thumbBuffer, mimeType: 'image/webp', suffix: '_thumb' };
  } catch (err) {
...
module.exports = { uploadToS3, getPublicUrl, generateThumbnail };
